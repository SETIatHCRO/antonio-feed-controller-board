...
void debug_command(char *args[])
{
    ...
    snprintf(msg,98,"%d_%d_(%u_%u)",m1,m2,last_chr_dbg_req, last_chr_dbg_resp);
    send_to_rimbox(msg);
    send_to_rimbox(LINESEP);
    snprintf(msg,98,"vof: %u cof: %u",debug_vac_overflow_cnt,debug_cryo_overflow_cnt);
    send_to_rimbox(msg);
    send_to_rimbox(LINESEP);
    s1 = U2STA;
    s2 = U3STA;
    snprintf(msg,98,"v_sta: 0x%x c_sta: 0x%x",s2,s1);
    send_to_rimbox(msg);
    send_to_rimbox(LINESEP);
    s1 = U2TXREG;
    s2 = U3TXREG;
    snprintf(msg,98,"v_txreg: 0x%x c_txreg: 0x%x",s2,s1);
    send_to_rimbox(msg);
    send_to_rimbox(EOL);
}
