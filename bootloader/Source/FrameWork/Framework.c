...
//		    WriteHexRecord2Flash(&RxBuff.Data[1], RxBuff.Len-3);  // Negate length of command and CRC RxBuff.Len.
		    WriteHexRecord2Flash(&RxBuff.Data[1], RxBuff.Len-1);  // no CRC
                    //Set the transmit frame length.
            TxBuff.Len = 1; // Command	    	
		   	break;
...
void BuildRxFrame(UINT8 *RxData, INT16 RxLen)
{
//  because BuildRxFrame is called internally, framing (SOH ... EOT) and
//  CRC are not required
        memcpy(RxBuff.Data, RxData, RxLen);
        RxBuff.Len = RxLen;
        RxFrameValid = TRUE;
//
...
UINT GetTransmitFrame(UINT8* Buff)
{
//  because GetTransmitFrame is called internally, framing (SOH ... EOT)
//  and CRC are not required
        memcpy(Buff, TxBuff.Data, TxBuff.Len);
        return(TxBuff.Len);
//
//      INT BuffLen = 0;
//	WORD_VAL crc;
