...
static unsigned char symbolto16(char in);

// saved 0xa4 bytes switching to this code rather
// than lookup table.  Things are getting tight enough
// in bootloader flash that we may need it.
static unsigned char symbolto16(char in) {
    if (isupper(in)) {
        return (in - 'A');
    }
    ...
    // should not get here.  Do something about it if it happens?
    return;
}
...
//                out[outi] = (decode_tbl[*in] << 2) & 0xfc;
                out[outi] = (symbolto16(*in) << 2) & 0xfc;
                break;
            case 1:
//                out[outi] |= ((decode_tbl[*in] >> 4) & 0x03);
                out[outi] |= ((symbolto16(*in) >> 4) & 0x03);
                outi++;
//                out[outi] = ((decode_tbl[*in] << 4) & 0xf0);
                out[outi] = ((symbolto16(*in) << 4) & 0xf0);
                break;
            case 2:
//                out[outi] |= ((decode_tbl[*in] >> 2) & 0x0f);
                out[outi] |= ((symbolto16(*in) >> 2) & 0x0f);
                outi++;
//                out[outi] = ((decode_tbl[*in] << 6) & 0xc0);
                out[outi] = ((symbolto16(*in) << 6) & 0xc0);
                break;
            case 3:
//                out[outi] |= (decode_tbl[*in] & 0x3f);
                out[outi] |= (symbolto16(*in) & 0x3f);
                outi++;
                break;
        }
        state = (state + 1) & 0x03;
