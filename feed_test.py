...
com_port = None
ports = ['/dev/ttyUSB0', 'COM4', 'COM3', 'COM2', 'COM1']
...
def open_com_port():
    for port in ports:
        try:
            ...
            com_port = Serial(port, 19200, timeout=0.05)
            print('O.K.')
            return (com_port)
        except Exception:
            print('fail')
...
def main():
    ...
    pathname = os.path.join(logdir,filename)

    if not os.path.exists(logdir):
        os.makedirs(logdir)
        print('directory {} does not exist, creating'.format(logdir))
    ...
    while (not (done)):
        if (kbhit() or newdata):
            if sys.platform.startswith('linu'):
                #we are on linux machine, doing workaround
                if newdata:
                    #we havent processed fully the previous call
                    keycode = ord(newdata.pop(0))
                    term_io_func_ptr(keycode)
                else:
                    #new or last call processed:
                    newdata = list(sys.stdin.readline())
            else:
                keycode = ord(getch())
                term_io_func_ptr(keycode)
